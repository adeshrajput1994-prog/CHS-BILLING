...
const PurchaseInvoiceForm: React.FC<PurchaseInvoiceFormProps> = ({
  ...
  useEffect(() => {
    console.log("PurchaseForm: Loading data from localStorage...");
    const storedFarmers = localStorage.getItem("farmers");
    if (storedFarmers) {
      setAllFarmers(JSON.parse(storedFarmers));
      console.log("PurchaseForm: Loaded farmers:", JSON.parse(storedFarmers));
    }
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
              <div className="space-y-2 print-hide">
                ...
                <Select onValueChange={(value) => purchaseForm.setValue("selectedFarmerId", value, { shouldValidate: true })} value={selectedFarmerId}>
                ...
                {purchaseForm.formState.errors.selectedFarmerId && (
                  <p className="text-red-500 text-sm">{purchaseForm.formState.errors.selectedFarmerId.message}</p>
                )}
              </div>
              {selectedFarmer ? (
                <>
